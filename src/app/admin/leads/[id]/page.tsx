...
export default function AdminLeadDetailPage() {
  ...
  const original = mockLeads.find((l) => l.id === params.id);
      ...
      <>
        ...
        <div className="flex-1 p-8">
          <p className="text-sm text-lf-muted">
            This lead doesn&apos;t exist in the demo data.{" "}
  ...
  function update<K extends keyof NonNullable<typeof lead>>(key: K, value: NonNullable<typeof lead>[K]) {
    setLead((l) => (l ? { ...l, [key]: value } : l));
  }

  function handleSave() {
    // NOTE: mock/local state only — no database connected yet.
    // When Supabase is connected, this becomes an update() call on the `leads` table.
    update("notes", notes);
    setEditing(false);
    setSaved(true);
            ...
            <div className="rounded-2xl border border-lf-border bg-lf-card p-6">
              ...
              {editing ? (
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Field label="Name">
                    <TextInput value={lead.name} onChange={(e) => update("name", e.target.value)} />
                  </Field>
                  <Field label="Business">
                    <TextInput value={lead.business} onChange={(e) => update("business", e.target.value)} />
                  </Field>
                  <Field label="Phone">
                    <TextInput value={lead.phone} onChange={(e) => update("phone", e.target.value)} />
                  </Field>
                  <Field label="Email">
                    <TextInput value={lead.email} onChange={(e) => update("email", e.target.value)} />
                  </Field>
                  <Field label="Industry">
                    <TextInput value={lead.industry} onChange={(e) => update("industry", e.target.value)} />
                  </Field>
                  <div className="sm:col-span-2">
                    <Field label="Problem">
                      <TextArea value={lead.problem} onChange={(e) => update("problem", e.target.value)} />
                    </Field>
                  </div>
                  <div className="sm:col-span-2">
                    <Field label="Time-consuming process">
                      <TextArea value={lead.process} onChange={(e) => update("process", e.target.value)} />
                    </Field>
                  </div>
                </div>
              ) : (
                <dl className="mt-6 grid grid-cols-1 gap-5 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <dd className="mt-1 text-sm leading-relaxed text-lf-ink">{lead.problem}</dd>
                  </div>
                  <div className="sm:col-span-2">
                    <dt className="text-xs font-semibold uppercase tracking-wide text-lf-muted">
                      Time-Consuming Process
                    </dt>
                    <dd className="mt-1 text-sm leading-relaxed text-lf-ink">{lead.process}</dd>
                  </div>
                </dl>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-lf-border bg-lf-card p-6">
              ...
              <div className="mt-4 flex flex-col gap-2">
                {leadStatuses.map((s) => (
                  <button
                    key={s}
                    onClick={() => update("status", s as LeadStatus)}
